...
/**
  * Creates a new iframe as the form target if it doesn't exist yet and submits the form
  */
function sendForm()
{
  setStatus("Sending...", 2/4);
  if ( !$("iframe[name=ifrUpload]")[0] )
    $( "#main" ).append("<iframe name='ifrUpload' width='1px' height='1px' style='display: none'></iframe>");
  ...
  readFile(file, function(file, evt)
  {
    ...
    $.ajax(
    {
      ...
      success: function(response)
      {
        ...
        try
        {
          ...
          if ( json['file_uploaded'] == true )
          {
            ...
            else // No error
            {
              ...
              {
                setStatus("File not correctly uploaded!", 1, true);
              }
            }     
          }
        ...
        {
          setStatus("Server returned invalid JSON!"+e, 1, true);
        }
      }
    }); // End AJAX
  });  // End readFile()
...
$(function()
{
  $( "#main" ).dialog( {modal:false, minWidth: 400} );
  ...
  {          
    ...
    {
      // If the File API is not supported but the users still wished to send the file via form and iframe
    ...
    {
      if ( $("#cbUseIFrame:checked").val() == undefined ) // If the user doesn't want to use iframes
      {
        setStatus("File too large (maxSize is "+maxSize+")! Activate the IFrame option.", 1, true);
      }
      else // Send the form (create a new iframe if it doesn't exist yet and submit the form)
      {
...
/**
  ...
  * @param msg The message to display in text format. It may contain newline characters which will be replaced by <br />.
  * @param value The new value for the progressbar between 0 and 1. It is multiplied by the `max` attribute of the progressbar
  * @param error If TRUE the statusWrapper div will get the `ui-state-error` style, otherwise the `ui-state-highlight` style from jQuery UI.
  */
