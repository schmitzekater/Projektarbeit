...
CREATE TABLE IF NOT EXISTS `GenBank`.`Pat` (
  ...
  `MutP` INT NOT NULL,
  PRIMARY KEY (`idPat`),
  INDEX `fk_Patient_MutP_idx` (`MutP` ASC),
...
CREATE TABLE IF NOT EXISTS `GenBank`.`Genname` (
  `idGenname` INT NOT NULL,
  `MDat_idMDat` INT NOT NULL,
  PRIMARY KEY (`idGenname`),
  INDEX `fk_Genname_MutDat1_idx` (`MDat_idMDat` ASC),
  CONSTRAINT `fk_Genname_MutDat1`
    FOREIGN KEY (`MDat_idMDat`)
    REFERENCES `GenBank`.`MutDat` (`idMDat`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
