...
<div id="wrapper">
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($nutzername) && !empty($email)) {
				...
				// Abfrage erfolgreich, also Daten an E-Mail senden und Sicherheitscode in die Datenbankspeichern inkl. Code generieren.	  
					...
					"wenn Sie ihr Passwort vergessen haben besuchen Sie bitte http://localhost/zuruecksetzen.php und ändern Sie ihr vergessenes Passwort in ein neues gewünschtes Passwort um.\n" .
					...
					echo '<p class="pass">Sie haben einen entsprechenden Sicherheitscode an Ihre E-Mail Adresse erhalten. Sie können <a href="zuruecksetzen.php">hier</a> Ihr Passwort zurücksetzen.</p>';
					
					mysqli_close($db);
				}
				else {
					echo '<p class="fail">E-Mail und Nutzername stimmen nicht überein.</p>';
				}
			}
			else {
	...
	<h2>Passwortänderung anfordern</h2>
