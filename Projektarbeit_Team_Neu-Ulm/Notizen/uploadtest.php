...
// Zeilenumbruch entfernen und erste Zeile aus Datei als Ãberschrift
...
foreach ($array as $line)// Aufteilen des Arrays in Zeilen
{
	...
	for ($i = 0; $i < count($elements); ++$i) {
		...
		// Ausgabe Ãberschrift: Element
	}
	$mysqli = connectDB($server, $user, $password, $dbase);		//Verbindung zur DB aufbauen
	if ($mysqli -> ping()) {									//Verbindung noch aktiv?
		$genId = checkGen($mysqli, $gen, $genTable);			//PrÃŒfe, ob das Gen bereits in der Datenbank ist.
...
function writeGenToDb($mysqli, $gen, $genTable) {
	...
	if($result = $mysqli->query($query))
	{
		...
		setStatus("Fehler beim EinfÃŒgen in die Datenbank.\nQuery: " . $query . "\n" . mysql_error() . "\n");
	}
	$mysqli->close();
	return $id;
}
...
function writeMutToDb($mysqli, $elements, $table, $genId) {
	...
	if($result = $mysqli->query($query))
	{
		setStatus("GeÃ€nderte Zeilen: " . $mysqli->affected_rows . "\n");
	}
	 else {
